...
const newFieldSchema = Joi.object({
    ...
    type: Joi.string().max(30).valid('Number').required(),
    ...
    // Defines the minimum value for number filed
    min: Joi.number(),

    // Defines the maximum value for number filed
    ...
    // // Defines default value if not provided
    // default: Joi.number()
    //     .when('min', {
    //         is: Joi.number().required(),
    //         then: Joi.number().min(Joi.ref('min')).max(Joi.ref('max')),
    //         otherwise: Joi.number().max(Joi.ref('max')),
    //     }),
});
